...
import { FilterAlt, ConnectingAirports } from '@mui/icons-material';
...
import { filterAllAirlines, sortAllPrice } from '../actions';


const AllFlights = () => {
	const allFlights = useSelector(state => state.filterFlights);
	const dispatch = useDispatch();

	const [filterAirlines, setFilterAirlines] = useState(AIRLINES[0].id);
	const [sortPrice, setSortPrice] = useState(5000);

	function handleAirlineChange(e) {
		setFilterAirlines(e.target.value);
		dispatch(filterAllAirlines(e.target.value));
	}

	const handlePriceChange = (event, newValue) => {
		dispatch(sortAllPrice(sortPrice));
		...
		<Box className='flight-parent-container'>
			<Paper elevation={12} className='flight-child-container'>
				...
				<Box
					...
					sx={{ margin: allFlights !== undefined && allFlights.length !== 0 ? 'auto' : 'auto' }}
				>
					{
						allFlights !== undefined && allFlights.length === 0 && 
						<Typography
							sx={{
								fontFamily: 'Poppins',
								fontWeight: 600,
					...
					{
						allFlights !== undefined && allFlights.length !== 0 && allFlights.map((el) => {
